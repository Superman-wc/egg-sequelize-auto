  ...
  run(callback) {
    ...
    function generateText(err) {
      ...
      async.each(_.keys(self.tables), function(table, _callback) {
        ...
        text[table] += spaces + "const Model = app.model."+self.sequelize.config.database+".define('" + tableName + "', {\n";
        ...
        text[table] +="createdAt: 'date_created',\n"
        text[table] +="updatedAt: 'last_updated',"
